    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans text-slate-900 overflow-x-hidden relative pb-24">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 relative z-10">
        ...
        <div className="transform lg:-rotate-1 hover:rotate-0 transition-all duration-700 perspective-1000 mb-12">
          <GlassCard className="p-8 md:p-12 group hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] transition-all flex flex-col md:flex-row items-center justify-between gap-8 bg-gradient-to-br from-white/40 to-orange-50/40 border-orange-200/50">
            <div className="w-full md:w-auto flex-1">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 text-center md:text-left">Your Referral Code</h2>
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <div className="w-full sm:w-auto flex-1 bg-white/80 backdrop-blur-md rounded-2xl border-2 border-orange-300/50 px-6 py-4 text-3xl md:text-4xl font-black text-center tracking-[0.3em] text-[#0f172a] font-mono select-all shadow-inner">
                  {referralCode}
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          <GlassCard className="p-6 md:p-8 flex flex-col justify-center group hover:-translate-y-2 transition-all duration-500">
            ...
            <p className="text-4xl md:text-5xl font-black text-[#0f172a] tracking-tighter leading-none">{stats.total_referrals || 0}</p>
          </GlassCard>
          ...
          <GlassCard className="p-6 md:p-8 flex flex-col justify-center group hover:-translate-y-2 transition-all duration-500 border-yellow-200/50 bg-yellow-50/20">
            ...
            <p className="text-4xl md:text-5xl font-black text-[#0f172a] tracking-tighter leading-none">{stats.pending_referrals || 0}</p>
          </GlassCard>
          ...
          <GlassCard className="p-6 md:p-8 flex flex-col justify-center group hover:-translate-y-2 transition-all duration-500 border-green-200/50 bg-green-50/20">
            ...
            <p className="text-4xl md:text-5xl font-black text-[#0f172a] tracking-tighter leading-none">{stats.completed_referrals || 0}</p>
          </GlassCard>
          ...
          <GlassCard className="p-6 md:p-8 flex flex-col justify-center group hover:-translate-y-2 transition-all duration-500 border-orange-200/50 bg-orange-50/20">
            ...
            <p className="text-3xl md:text-4xl font-black text-[#0f172a] tracking-tighter leading-none text-orange-600">
              <span className="text-base text-orange-500 mr-1">KES</span>
        ...
        {currentBalance > 0 && (
          <div className="p-1 bg-gradient-to-br from-green-400 via-emerald-300 to-teal-400 rounded-[2.5rem] shadow-2xl mb-12 transition-all hover:scale-[1.01]">
            <div className="h-full w-full bg-white/95 backdrop-blur-3xl rounded-[2.4rem] p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
              ...
              <div>
                <p className="font-black text-2xl md:text-3xl text-[#0f172a] tracking-tighter uppercase mb-1">
                  KES {currentBalance.toLocaleString()} <span className="text-green-500">Available</span>
                </p>
                <p className="text-sm font-bold text-slate-500">This credit will be automatically deducted from your next order total.</p>
              </div>
            </div>
          </div>
        )}
        ...
        <GlassCard className="mb-12 !p-0 overflow-hidden">
          <div className="px-8 py-6 border-b border-white/50 bg-white/30 backdrop-blur-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-black text-[#0f172a] uppercase tracking-tighter">Your Network</h2>
              <p className="text-sm font-bold text-slate-500 mt-1">Track their progress towards earning you KES 50.</p>
            </div>
            <Users size={32} className="text-slate-300 hidden sm:block" />
          </div>
                    ...
                    {referredUsers.map((ref) => (
                      <tr key={ref.id} className="hover:bg-white/40 transition-colors">
                        <td className="px-6 py-5">
                          <p className="text-sm font-black text-[#0f172a]">{ref.referee_name}</p>
                          <p className="text-[10px] font-bold text-slate-500 mt-1">{ref.referee_email}</p>
                        </td>
        ...
        <div className="relative group overflow-hidden rounded-[2.5rem] bg-[#0f172a] p-8 md:p-12 text-white shadow-2xl flex flex-col transition-all hover:scale-[1.01]">
