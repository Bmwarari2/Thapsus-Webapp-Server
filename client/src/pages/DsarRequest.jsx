    ...
    <div className="relative min-h-screen bg-white/[0.03]">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 md:px-8 py-10">
        ...
        <GlassCard className="p-6 mb-6">
          <h3 className="text-lg font-black text-white mb-4">New request</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            <label className={`flex items-center gap-3 p-4 rounded-2xl cursor-pointer border-2 transition-all
                              ${type === 'export' ? 'border-orange-500 bg-ember-500/10' : 'border-line bg-surface-2'}`}>
              ...
              <div>
                ...
                <p className="text-xs text-mute">Receive a JSON copy of every record we hold.</p>
              </div>
            </label>
            <label className={`flex items-center gap-3 p-4 rounded-2xl cursor-pointer border-2 transition-all
                              ${type === 'erase' ? 'border-orange-500 bg-ember-500/10' : 'border-line bg-surface-2'}`}>
              ...
              <div>
                ...
                <p className="text-xs text-mute">We may retain records required by HMRC / KRA.</p>
              </div>
            </label>
          </div>
        ...
        <h3 className="text-lg font-black text-white mb-3">Your past requests</h3>
        {requests.length === 0 ? (
          <GlassCard className="p-8 text-center text-mute">No DSARs submitted yet.</GlassCard>
        ) : (
          <div className="space-y-3">
            {requests.map(r => (
              <GlassCard key={r.id} className="p-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-mono text-xs text-mute">{r.id}</p>
                  <p className="font-semibold text-white capitalize">{r.type}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {r.export_url && <a className="text-xs text-ember-400 hover:underline" href={r.export_url} target="_blank" rel="noreferrer">Download</a>}
                </div>
              </GlassCard>
            ))}
          </div>
