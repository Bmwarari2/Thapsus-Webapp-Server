...
import {
  Package, Truck, Calculator, Layers, LogOut, User, DollarSign,
  CheckCircle, Clock, AlertTriangle, ChevronRight,
  Menu, X, Box, BarChart3, Users, Globe, ShieldCheck,
  ArrowRight, MapPin, RefreshCcw, Gift, HelpCircle,
...
const WAREHOUSE_ADDRESSES = {
  UK: {
    ...
    note: "Include your SC-ID in Name field"
  },
  USA: {
...
const RETAILERS = ["Amazon", "Shein", "Next", "Asos", "Superdrug", "eBay", "Walmart", "Zara"];
...
const simulatedApi = async (endpoint) => {
  await new Promise(res => setTimeout(res, 500));
  if (endpoint === '/orders') return {
    orders: [
      { id: 'ORD-101', tracking_number: 'TBA309482', retailer: 'Amazon', market: 'USA', status: 'received_at_warehouse' },
      { id: 'ORD-102', tracking_number: 'SHN992011', retailer: 'Shein', market: 'China', status: 'in_transit' },
    ]
  };
  return { success: true };
};
...
const Button = ({ children, variant = 'primary', className = '', ...props }) => {
  const base = "inline-flex items-center justify-center px-6 py-3 font-bold rounded-lg transition-all active:scale-95 disabled:opacity-50";
  const variants = {
    ...
    <footer className="bg-slate-900 text-white py-16 px-6">
      <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12">
        ...
        <div>
          ...
          <div className="space-y-3">
            <p className="text-slate-300 font-medium">United Kingdom</p>
            <p className="text-slate-300 font-medium">United States</p>
            <p className="text-slate-300 font-medium">China</p>
          </div>
        </div>
        <div>
          ...
          <Card key={i} className="text-center flex flex-col items-center">
            ...
            <div className="w-full pt-6 border-t border-slate-100">
               ...
               <div className="flex flex-wrap justify-center gap-2">
                 {r.items.map(item => <span key={item} className="px-3 py-1 bg-slate-100 rounded-full text-xs font-bold text-slate-600">{item}</span>)}
               </div>
            </div>
          </Card>
        ))}
...
export default function App() {
  ...
  const login = async (email) => {
    ...
    setUser({
      id: isAdmin ? 'admin-1' : 'user-5421',
      name: isAdmin ? 'Swift Admin' : 'Jane Nyambura',
      role: isAdmin ? 'admin' : 'customer',
      warehouse_id: 'SC-5421'
    });
  ...
  const renderContent = () => {
    switch (currentPath) {
              ...
              <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4">
                  ...
                  <div className="flex gap-1">
                    {[1,2,3].map(i => <div key={i} className="w-2 h-2 rounded-full bg-blue-500" />)}
                  </div>
                </div>
      ...
      case '/login': return <LoginPage login={login} />;
      default: return user ? <DashboardShell user={user} navigate={navigate} path={currentPath} logout={logout} /> : <LoginPage login={login} />;
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <Card className="max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500">{isSignUp ? 'Start shipping from the world to Kenya' : 'Login to manage your international shipments'}</p>
          </div>
          <div className="space-y-4">
            ...
            <Button className="w-full h-14 text-lg" onClick={() => login(email || 'user@test.com')}>
...
const DashboardShell = ({ user, navigate, path, logout }) => {
  ...
  useEffect(() => {
    simulatedApi('/orders').then(res => setOrders(res.orders));
  }, []);

  const rates = { UK: { rate: 12.50, currency: 'GBP', toKES: EXCHANGE_RATES.GBP_TO_KES }, USA: { rate: 15.00, currency: 'USD', toKES: EXCHANGE_RATES.USD_TO_KES }, CHINA: { rate: 9.00, currency: 'USD', toKES: EXCHANGE_RATES.USD_TO_KES } };

  const calculateShipping = () => {
    const r = rates[calcMarket];
    const foreign = r.rate * calcWeight;
    const kes = foreign * r.toKES;
    setCalcResult({ foreign: `${r.currency === 'GBP' ? '£' : '$'}${foreign.toFixed(2)}`, kes: `KES ${kes.toLocaleString('en-KE', { minimumFractionDigits: 2 })}` });
  };
             ...
             <div className="space-y-8">
               <Card className="bg-blue-700 text-white border-none relative overflow-hidden">
                 <div className="relative z-10">
                   ...
                   <p className="text-blue-100 opacity-80">Use your unique ID <span className="font-mono bg-white/20 px-2 py-0.5 rounded">{user.warehouse_id}</span> for all shopping.</p>
                        ...
                        {orders.map(order => {
                          ...
                          return (
                            <tr key={order.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-4">
                                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${statusStyle.bg} ${statusStyle.text}`}>
               ...
               <div className="grid md:grid-cols-2 gap-8">
                 <Card>
                   ...
                   <div className="space-y-5">
                      ...
                      <Button className="w-full py-4 mt-2" onClick={calculateShipping}>
                ...
                <Card>
                   <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-6">
                        ...
                        <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase mb-1">City / State / Zip</p>
                      ...
                      <div className="flex flex-col justify-between">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-amber-700 text-xs mt-1">
                                Always include your SC-ID <span className="font-mono font-bold">{user.warehouse_id}</span> in the Name field when shopping. This helps us identify your packages.
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        >
                          {copySuccess ? (
                            <><CheckCircle size={18} className="mr-2" /> Copied!</>
                          ) : (
                            <>Copy Full Address</>
                          )}
                        </Button>
           ...
           {path === '/admin' && (
             <div className="space-y-8">
               ...
               <Card>
                 ...
                 <div className="flex gap-4">
                   <input className="flex-1 p-4 bg-slate-50 rounded-xl font-bold border border-slate-200 outline-none focus:ring-2 focus:ring-blue-600" placeholder="Scan or Enter Tracking Numbers (comma separated)" />
                   <Button>
